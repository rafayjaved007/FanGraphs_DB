...
for player_url, player in zip(urls, players):
    try:
        ...
        b_url = driver.find_element_by_xpath(
            '//div[contains(@id,"content")]//ul[contains(@class,"menu-player-page__batpitch")]/li[1]//div/a').get_attribute("href")
        driver.get(b_url)
        ...
        for date, team, opp, bo, pos, g, ab, pa, h, one_b, two_b, three_b, hr, r, rbi, bb, ibb, so, hbp, sf, sh, gdp,\
            ...
            requests.post(url='http://127.0.0.1:8000/core/dual-hitters-standard/', data=json.dumps(data), headers=headers)
